...
module("Unit | Initializer | ember perf timeline", function() {
  ...
  test('instrumentationsFromSearch', function(assert) {
    assert.equal(instrumentationsFromSearch(''), '', 'Empty search returns empty');
    assert.equal(instrumentationsFromSearch('?a=1'), '', 'Without _ember-perf-timeline returns empty')
    assert.equal(instrumentationsFromSearch('?_ember-perf-timeline=true&a=1'), 'true', 'returns value')
    assert.equal(instrumentationsFromSearch('?_ember-perf-timeline=render.component,render.outlet&a=1'), 'render.component,render.outlet', 'returns value separated by ,')
    assert.equal(instrumentationsFromSearch('?a=1&_ember-perf-timeline=false'), 'false', 'returns value when not at first param')
  })
});
